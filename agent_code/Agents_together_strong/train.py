...
import events as e
import settings
from .callbacks import state_to_features, save_model, check_all_paths_for_route, \
    escape_combinations, FEATURE_SIZE, get_blast_coords
...
def game_events_occurred(self, old_game_state: dict, self_action: str,
    ...
    crate, escape, suicide = check_placed_bomb(new_game_state, events)
...
def reward_coin_distance(old_game_state, new_game_state, events):
    ...
    # TODO only if player collected coin not enemy!
    if len(old_coins) == len(new_coins) and len(old_coins) != 0:
        # Look at coins that are persistent
        # (with the same coordinates in both states)

        persistent_coins = old_coins[np.all(np.isin(old_coins, new_coins))]
        if len(persistent_coins) == 0:
            # TODO fix this
            return
        else:
        ...
        # Find the index of the previous closes coin
    ...
    elif len(old_coins) != len(new_coins):
        events.append(CLOSER_TO_COIN)
...
def check_placed_bomb(new_game_state, events):
    ...
    if "BOMB_DROPPED" in events:
        for (bomb_x, bomb_y), timer in new_game_state["bombs"]:
            if [bomb_x, bomb_y] == [player_x, player_y]:

                escapable_bomb = np.any([
                    check_all_paths_for_route(np.array(route), field, player_x,
                                              player_y)
                    for route in escape_combinations
                ])
                if not escapable_bomb:
                    suicide_bomb = True
                for x in range(-3, 4):
                    if bomb_x + x >= 17 or bomb_x + x < 0:
                        continue
                    if field[bomb_x + x, bomb_y] == 1:
                        destroy_crate = True
                        return destroy_crate, escapable_bomb, suicide_bomb
                for y in range(-3, 4):
                    if bomb_y + y >= 17 or bomb_y + y < 0:
                        continue
                    if field[bomb_x, bomb_y + y] == 1:
                        destroy_crate = True
                        return destroy_crate, escapable_bomb, suicide_bomb
    return destroy_crate, escapable_bomb, suicide_bomb

...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if state_to_features(old_game_state) is not None:
        ...
        #Update Q_matrix
        #print(old_state.shape, old_state)
        old_value = self.Q_table[old_state[0], old_state[1], old_state[2],old_state[3], action]

        next_max = np.max(self.Q_table[new_state[0], new_state[1], new_state[2],new_state[3], action])
        reward = reward_from_events(self, events)

        new_value = (1 - self.alpha) * old_value + self.alpha * (reward + self.gamma * next_max)

        self.Q_table[old_state[0], old_state[1], old_state[2],old_state[3], action] = new_value
